  ...
  if (withCoords.length === 0) {
    return (
      <div style={{
        ...
        <p style={{ fontFamily: 'var(--font-crimson), Georgia, serif', fontSize: 18, color: T.espresso }}>No location data yet</p>
        <p style={{ fontFamily: 'var(--font-dm-mono), ui-monospace, monospace', fontSize: 12, color: T.mist, letterSpacing: '0.06em', textAlign: 'center', maxWidth: 280 }}>
  ...
  if (!isLoaded) {
    return (
      <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: T.linen, border: `0.5px solid ${T.border}`, borderRadius: 10 }}>
        <p style={{ fontFamily: 'var(--font-dm-mono), ui-monospace, monospace', fontSize: 12, color: T.mist, letterSpacing: '0.06em' }}>loading map…</p>
      </div>
    )
  }
      ...
      {selected && (
        ...
        >
          ...
          <div style={{ padding: '14px 14px 10px', flexShrink: 0, position: 'relative' }}>
            ...
            <p style={{ fontFamily: 'var(--font-crimson), Georgia, serif', fontSize: 19, fontWeight: 400, color: T.espresso, paddingRight: 32, margin: 0, lineHeight: 1.2 }}>
              {displayName}
            </p>
            {selected.cuisine && (
              <p style={{ fontFamily: 'var(--font-dm-mono), ui-monospace, monospace', fontSize: 11, color: T.mist, marginTop: 4, letterSpacing: '0.06em' }}>
                {selected.cuisine}
              </p>
            )}
            {(selected.address || selected.suburb) && (
              <p style={{ fontFamily: 'var(--font-dm-mono), ui-monospace, monospace', fontSize: 11, color: T.ghost, marginTop: 2, letterSpacing: '0.04em' }}>
              ...
              <div style={{ display: 'flex', gap: 8, marginTop: 6, flexWrap: 'wrap' }}>
                {selected.website && (
                  <a href={selected.website} target="_blank" rel="noopener noreferrer"
                    style={{ display: 'flex', alignItems: 'center', gap: 4, fontFamily: 'var(--font-dm-mono), ui-monospace, monospace', fontSize: 11, color: T.terracotta, letterSpacing: '0.04em', textDecoration: 'none' }}>
                ...
                {selected.instagram && (
                  <a href={`https://www.instagram.com/${selected.instagram.replace(/^@/, '')}/`} target="_blank" rel="noopener noreferrer"
                    style={{ display: 'flex', alignItems: 'center', gap: 4, fontFamily: 'var(--font-dm-mono), ui-monospace, monospace', fontSize: 11, color: T.mist, letterSpacing: '0.04em', textDecoration: 'none' }}>
          ...
          {selected.visit_count > 0 && (visitsLoading || visits.length > 0) && (
            <div style={{ borderTop: `0.5px solid ${T.border}`, flex: 1, overflowY: 'auto' }}>
              <p style={{ fontFamily: 'var(--font-dm-mono), ui-monospace, monospace', fontSize: 10, color: T.mist, letterSpacing: '0.1em', textTransform: 'uppercase', padding: '10px 14px 6px' }}>
                visits{visits.length > 0 ? ` · ${visits.length}` : ''}
              </p>
              {visitsLoading ? (
                <p style={{ fontFamily: 'var(--font-dm-mono), ui-monospace, monospace', fontSize: 11, color: T.ghost, padding: '0 14px 12px' }}>loading…</p>
              ) : (
                <div>
                  {visits.map((v, i) => (
